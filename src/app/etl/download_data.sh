...
mkdir -p /var/etl/data/

current_date=$(date +"%Y_%m_%d")
while read phila_alias
do
  curl "https://data.phila.gov/resource/tqtk-pmbv.csv?\$limit=50000&\$order=parcel_number&owner_1=${phila_alias}" | tail -n +2 >> /var/etl/data/${current_date}_city_owned.csv
